...
def sents_categories(*categories):
    ...
    return brown.sents(categories=categories), brown.tagged_sents(tagset=categories)


def words_categories(*categories):
    if not len(categories):
        return brown.words(), brown.tagged_words()
    return brown.words(categories=categories), brown.tagged_words(tagset=categories)
...
def ngrammes(words_text, n):
    ...
    """
    ngrams, d = [], words_text
    ngrams.append([[''.join(d[i:j]) for j in range(i+1,min(i+n+1,len(d)+1))]
