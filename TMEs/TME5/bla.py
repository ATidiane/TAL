...
def get_words(txt):
    t = filter(
        ...
        txt.replace(
            '-',
            ' ')).split()
    return filter(lambda w: ('@' not in w) and (
        not w[-3:] in ["edu", "com"]) and (w[:3] != "www"), t)
...
all_words = {
    k: v for (
        k, v) in filter(
            lambda c: c[1] > 15, zip(
                all_words.keys(), all_words.values()))}
# suppression des 200 premiers mots
all_words = {k: i for (k, i) in sorted(
    zip(all_words.keys(), all_words.values()), key=lambda k: k[1])[::-1][200:]}
...
def purity_of_one_cluster(y, y_hat, id_cluster):
    ...
    tmp = zip(*tmp)[0]
    c = Counter(tmp)
    nb_most_redundant = sorted(
        zip(c.keys(), c.values()), key=lambda c: c[1])[-1][1]
...
def n_mots_important_words_by_cluster_naive(components, n):
    return map(
        lambda c: n_most_important_words_one_component_naive(
            c, n), components)
...
def fusion(cat):
    if cat in ['alt.atheism', 'soc.religion.christian', 'talk.religion.misc']:
        return 'religion'
    elif cat in ['comp.sys.mac.hardware', 'comp.sys.ibm.pc.hardware',
                 'comp.graphics', 'comp.os.ms-windows.misc', 'comp.windows.x']:
        return 'computer'
    elif cat in ['rec.autos', 'rec.motorcycles']:
        return 'vehicule'
    elif cat in ['rec.sport.baseball', 'rec.sport.hockey']:
        return 'sport'
    elif cat in ['talk.politics.guns', 'talk.politics.mideast',
                 'talk.politics.misc']:
        return 'politic'
    else:
        return cat
