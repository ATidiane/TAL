...
def removeNum(doc):
    ...
    return re.sub(
        '[0-9]+',
        '',
        doc)  # remplacer une séquence de chiffres par rien
...
def categorize(path):
    """
    """
    rdr = pt.CategorizedPlaintextCorpusReader(
        path, '.*/[0-9]+', encoding='latin1', cat_pattern='([\w\.]+)/*')
    docs = [[rdr.raw(fileids=[f]) for f in rdr.fileids(c)]
...
def ngrammes(corpuspp, n):
    ...
    for t, topic in enumerate(corpuspp):
        ...
        for d in topic:
            d = d.split()
            ngrams[t].append([[''.join(d[i:j]) for j in range(
...
def countVectorizer(corpuspp):
    ...
    for t, topic in enumerate(corpuspp):
        corpus.append([])
        for d in topic:
            # on enleve les mots qui apparaissent moins de 5 fois
            min_df = 5. * 1. / len(d.split())
